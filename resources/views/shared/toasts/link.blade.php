                    ...
                    <div class="toast-body">
                        <div class="row">
                            <div class="col d-flex text-truncate">
                                <div class="{{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}"><img src="{{ favicon($link->url) }}" rel="noreferrer" class="width-4 height-4"></div>

                                <div class="text-truncate">
                                    <a href="{{ route('stats.overview', $link->id) }}" dir="ltr">{{ $link->displayShortUrl }}</a>

                                    <div class="text-dark text-truncate small">
                                        <span class="text-secondary cursor-help" data-toggle="tooltip-url" title="{{ $link->url }}">@if($link->title){{ $link->title }}@else<span dir="ltr">{{ $link->displayUrl }}</span>@endif</span>
                                    </div>
                                </div>
                            </div>
