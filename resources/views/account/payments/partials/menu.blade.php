...
<div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow">
    ...
    @if($payment->status == 'pending')
        @if(request()->is('admin/*'))
            ...
            <a class="dropdown-item text-success d-flex align-items-center" href="#" data-toggle="modal" data-target="#modal" data-action="{{ route('admin.payments.approve', $payment->id) }}" data-button-class="btn btn-success position-relative" data-title="{{ __('Approve') }}" data-text="{{ __('Are you sure you want to approve :name?', ['name' => $payment->payment_id]) }}">@include('icons.checkmark', ['class' => 'fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Approve') }}</a>
        @endif

        <div class="dropdown-divider"></div>
        <a class="dropdown-item text-danger d-flex align-items-center" href="#" data-toggle="modal" data-target="#modal" data-action="{{ request()->is('admin/*') ? route('admin.payments.cancel', $payment->id) : route('account.payments.cancel', $payment->id) }}" data-button-class="btn btn-danger position-relative" data-title="{{ __('Cancel') }}" data-text="{{ __('Are you sure you want to cancel :name?', ['name' => $payment->payment_id]) }}">@include('icons.close', ['class' => 'fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Cancel') }}</a>
    @endif
</div>