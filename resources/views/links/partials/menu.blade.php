...
<div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow">
    ...
    <a class="dropdown-item d-flex align-items-center" href="{{ ($link->shortUrl) }}" target="_blank" rel="nofollow noreferrer noopener">@include('icons.eye', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('View') }}</a>

    <a class="dropdown-item d-flex align-items-center" href="{{ ($link->shortUrl) }}/+" target="_blank">@include('icons.eye-preview', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Preview') }}</a>
    ...
    <a class="dropdown-item d-flex align-items-center link-share" href="#" data-toggle="modal" data-target="#share-modal" data-url="{{ ($link->shortUrl) }}" data-title="@if($link->title){{ $link->title }}@else{{ $link->displayUrl }}@endif" data-qr="{{ route('qr', $link->id) }}">@include('icons.share', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Share') }}</a>
