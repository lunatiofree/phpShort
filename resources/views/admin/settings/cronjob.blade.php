    ...
    <div class="card-body">
        ...
        <div class="form-group">
            ...
            <small class="form-text text-muted">
                {{ __('The cron job command must be set to run every minute.') }} @if(config('settings.cronjob_executed_at')) {{ __('Last executed at: :date.', ['date' => Carbon\Carbon::createFromTimestamp(config('settings.cronjob_executed_at'))->tz(Auth::user()->timezone ?? config('settings.timezone'))->format(__('Y-m-d') . ' H:i:s')]) }} @endif
            </small>
        </div>
