...
class CheckoutController
{
    ...
    public function process(ProcessCheckoutRequest $request, $id)
    {
        ...
        } elseif ($request->input('payment_processor') == 'mollie') {
            return $this->initMollie($request, $plan, $coupon, $taxRates, $amount);
        } elseif ($request->input('payment_processor') == 'paddle') {
            return $this->initPaddle($request, $plan, $coupon, $taxRates, $amount);
     ...
     * @param $taxRates
     * @param $amount
     * @return Factory|\Illuminate\Contracts\View\View|\Illuminate\Foundation\Application|RedirectResponse|object
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    private function initMollie(Request $request, Plan $plan, $coupon, $taxRates, $amount) {
        ...
        try {
            $mollieCustomerRequest = $client->post($httpBaseUrl . 'customers', [
                'headers' => [
        ...
        // Attempt to create the payment for the customer
        try {
            $molliePaymentRequest = $client->post($httpBaseUrl . 'customers/' . $mollieCustomer['id'] . '/payments', [
                'headers' => [
                    'Authorization' => 'Bearer ' . config('settings.mollie_key'),
                    'Content-Type' => 'application/json',
                ],
     ...
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    private function initPaddle(Request $request, Plan $plan, $coupon, $taxRates, $amount) {
        $client = new HttpClient();

        $httpBaseUrl = 'https://'.(config('settings.paddle_mode') == 'sandbox' ? 'sandbox-api' : 'api').'.paddle.com/';

        try {
            $paddleProductRequest = $client->request('POST', $httpBaseUrl . 'products', [
                'headers' => [
        ...
        $paddlePlan = $plan->id . '_' .$request->input('interval') . '_' . $paddleAmount . '_' . $plan->currency;

        try {
            $paddlePriceRequest = $client->request('POST', $httpBaseUrl . 'prices', [
                'headers' => [
                    'Authorization' => 'Bearer ' . config('settings.paddle_api_key'),
                    'Content-Type'  => 'application/json',
                    'Accept'        => 'application/json',
